...
public class RedSkystones extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        drive.followTrajectorySync(drive.trajectoryBuilder().forward(skybridge + skystonePosition).build());

        //TODO: Drop Stone

        /*
            drive.followTrajectorySync(drive.trajectoryBuilder().back(skybridge+skystonePosition).build());
        c = 0;
        while (!isStopRequested()) {
            camera.loop(true);
            try {
